...
int overall_signal_num = 0;
int global_Packet_ID = 0;
int global_adminInterval;
int global_respSignalNum;
int global_injSignalHighGroupNum = 0;
...
//
int find_outport(int current, int next) {
    switch (current - next) {
        case 1:
            return 3; // left
        case -1:
...
void initial_tdm_table() {
    ...
    outfile_router.open("copy_4/file_64/table_router.txt");
    outfile_NI.open("copy_4/file_64/table_ni.txt");
    traffic_mode.open("copy_4/file_64/traffic_mode.txt");
...
//总共运行6万次
    ...
    while(1) {  //开始每一个epoch
        ...
        if (int(cycles) % int(paramTotalSimCycle / parameter_NI_statePeriodNum) == 0 && int(cycles) >= 300 && int(cycles) < 50200) {//cycles = 0 ini//当cycles能被300整除 ，且 1200<cycles<5000的值时将一些参量送入RL
...
#ifdef param_RL_SwitchOn_inMain
            if (cycles < 50100 && cycles >= 300) { //0-300个cycles 不调节，使用0-300的网络运行参数作为状态调节
                for (int i = 0; i < 168; i++) {
// 245.4349979599389
...
// 245.4349979599389
        ...
        if (printfSW_main_averageHop == 1) {
...
#ifdef Param_RL_writeFilePakcetDelay_InMain
            write_file_packetdelay(1);
#else
            write_file_packetdelay(0);
#endif
           ...
           /* resetfile1.open("resetReady.txt", std::ios::out | std::ios::trunc);
            ...
            //
            //用于停止程序
            std::cout <<"#STOP_PROGRAM"<< std::endl;
